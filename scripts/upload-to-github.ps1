# Upload tomato-novel-writer skill to GitHub
...
foreach ($File in $Files) {
    ...
    $Body = @{
        message = "Add tomato-novel-writer skill - $($File.Path)"
    ...
    try {
        ...
        Write-Host "✅ $($File.Path) uploaded"
    } catch {
        Write-Host "❌ $($File.Path) failed: $($_.Exception.Message)"
    }
}
